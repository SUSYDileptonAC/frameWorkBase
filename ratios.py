...
class Ratio:
        def __init__(self, numerator, denominator, numeratorSquaredError, denominatorSquaredError, xPos, width):
                ...
                #self.numeratorSquaredErrorsUp = [numeratorSquaredError]
                #self.numeratorSquaredErrorsDn = [numeratorSquaredError]
        ...
        def errorY(self):
                value = 0.0
                if (self.isValid): # and self.sumNumerator > 0 <- not neccessary
                        # this value does not hold if ratio is zero
                        ...
                        value = math.sqrt(self.sumNumeratorSquaredErrors / math.pow(self.sumDenominator, 2.0) + self.sumDenominatorSquaredErrors * math.pow(self.sumNumerator/math.pow(self.sumDenominator, 2.0), 2.0) )
                #print value, self.sumNumeratorSquaredErrors
                return value
...
class RatioGraph:
        ...
        def getGraph(self):
                ratios = []
                #self.numerator.SetBinErrorOption(ROOT.TH1.kPoisson)
                tempRatio = None
                for iBin in range(1, 1 + self.numerator.GetNbinsX()):
                        ...
                        if num < den:
                                numError = self.numerator.GetBinErrorUp(iBin)
                        else:
                                numError = self.numerator.GetBinErrorLow(iBin)
                        #~ print "DEBUG RATIO: " 
                        #~ print iBin
                        #~ print num 
                        #~ print numError
                        ...
                        if (self.xMin < x and x < self.xMax
                                ...
                                if not den == 0:
                                        ...
                                        #~ else:
                                                #~ ratios.append(tempRatio)
                                                #~ tempratio = None
                                        if True: # (tempRatio.isFullEnough(self.adaptiveBinning)):
