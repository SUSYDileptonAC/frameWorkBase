...
def readTreeFromFile(path, dileptonCombination, modifier = ""):
	...
	if modifier == "":
		...
		if "Single" in modifier:
			result.Add("%s/cutsV24DileptonMiniAOD%sFinalTriggerTrees/%sDileptonTree"%( path, modifier, dileptonCombination))
		elif "Fake" in modifier:
			result.Add("%s/cutsV24DileptonMiniAOD%sTree/Trees/Iso"%( path, modifier))
		elif "baseTrees" in modifier:
			result.Add("%s/cutsV24DileptonMiniAODBaseTrees/%sDileptonTree"%(path, dileptonCombination))
		elif "deltaBetaReweighted" in modifier:
			result.Add("%s/cutsV24DileptonAODFinalTrees/%sDileptonTree"%(path, dileptonCombination))
		else:
			result.Add("%s/cutsV24DileptonMiniAOD%sFinalTrees/%sDileptonTree"%( path, modifier, dileptonCombination))
	return result
...
def getFilePathsAndSampleNames(path,source="",modifier = ""):
	...
	#~ for filePath in glob("%s/miniAODTriggerTest*.root"%path):
		#~ if source == "":
			#~ sampleName = match(".*miniAODTriggerTest.*\.processed.*\.(.*).root", filePath).groups()[0]		
		#~ else:
			#~ sampleName = ""
